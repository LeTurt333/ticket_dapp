...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::InitTicketContract { code_id, name, symbol } => execute_init_ticket(deps, env, &info.sender, code_id, name, symbol),

        ExecuteMsg::BuyTicket { package_option } => execute_buy_ticket(deps, info, env, package_option),

        ExecuteMsg::ReceiveNft(receive_nft_msg) => execute_receive_nft(deps, env.block.height, info, receive_nft_msg),
    }
}
...
pub fn execute_buy_ticket(
    ...
    match package_option {
        ...
        3 => {
            ...
            if !has_coins(&info.funds, &coin(ULTRA_VIP_PRICE.u128(), TICKET_PRICE_DENOM)) {
                return Err(ContractError::InvalidPayment {  });
            }
            if info.funds.len() > 1 {
                return Err(ContractError::GenericError {  });
...
pub fn execute_receive_nft(
    ...
    if (used_tickets().idx.ticket_id.item(deps.storage, wrapper.token_id.clone())?).is_some() {
        return Err(ContractError::GenericError{});
    }

    // Optionally check if sender has already used a ticket
    if {
        let used_tickets: StdResult<Vec<_>> = used_tickets().prefix(&user_wallet).range(deps.storage, None, None, Order::Ascending).collect();
    ...
    match msg {
        ReceiveNftMsg::UseTicket {} => use_ticket(deps, block, user_wallet, wrapper.token_id, info.sender.to_string())
    }
}
    ...
    match msg.id {
        ...
        1 => {
            ...
            return Ok(Response::new()
                .add_attribute("Instantiated NFT Ticket contract", format!("Address: {}", ticket_contract.to_string()))
            );
        },
        ...
        2 => {
            ...
            return Ok(Response::new()
                .add_attribute("Mint Ticket", "Success")
            );
        },
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::GetUserStatus { user_address } => to_binary(&get_user_status(deps, user_address)?)
    }
}
...
pub fn get_user_status(
    ...
    let checked: StdResult<Vec<(_, TicketInfo)>> = used_tickets().prefix(&user_wallet).range(deps.storage, None, None, Order::Ascending).collect();
    ...
    let resp: NftInfoResponse<TicketNftExtension> = deps
        .querier
        .query(&QueryRequest::Wasm(
            WasmQuery::Smart{
                contract_addr: nft_addr.to_string(),
                msg: to_binary(&format!("NftInfo {{ token_id: {} }}", checked_tickets[0].1.ticket_id))?
            }
        ))?;
    ...
    let traits: Vec<_> = attributes.iter().map(|t| format!("{}: {}", t.trait_type, t.value)).collect();
